...
The code chunk below will show the summary of the dataset.
...
### `read_csv` is built in in RStudio, but `read_excel` is not. That’s why we need to load the package `readxl` at the beginning.
...
``` r
write_csv(lotr_words, file = "Results/lotr_words.csv")
```
